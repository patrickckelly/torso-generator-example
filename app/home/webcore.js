...
define(["underscore", "jquery", "backbone", "handlebars", "backbone-nested", "backbone-stickit"], function(_, $, Backbone, Handlebars) {
return factory(root, {}, _, $, Backbone, Handlebars);
});
} else if (typeof exports === "object") {
require("backbone");require("backbone-nested");require("backbone.stickit");module.exports = factory(root, {}, require("underscore"), require('jquery'), require("backbone"), require("handlebars"));
} else {
...
WebCore.Mixins.Validation = WebCore.Validation.mixin;

/* Stickit configuration */
WebCore.Stickit = require('backbone.stickit');
 ...
 * @author    ariel.wexler@vecna.com
 */
WebCore.TemplateRenderer = (function() {
  /**
  ...
  function hotswap(currentNode, newNode, ignoreElements) {
    ...
    if(newNodeType !== currentNodeType) {
      WebCore.$(currentNode).replaceWith(newNode);
    } else {
  ...
  function swapElementNodes(currentNode, newNode, ignoreElements) {
    var $currentNode = WebCore.$(currentNode),
      $newNode = WebCore.$(newNode),
      shouldIgnore,
      $currChildNodes,
      $newChildNodes,
      currentAttributes;
  ...
  return {
    ...
    render: function($el, template, context, opts) {
      ...
      if (opts.force) {
        ...
        newDOM = WebCore.$('<' + $el.prop('tagName') + '>').html(newHTML);
...
WebCore.Views.Form = WebCore.View.extend({
  ...
  delegateEvents: function() {
    ...
    this.stickit();
    WebCore.View.prototype.delegateEvents.call(this);
